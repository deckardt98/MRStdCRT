...
robust_CRT_point <- function(formula, data, clus_id, trt, prob, 
  ...
  if (method %in% c("LM", "LMM")) {
    if (family$family != "gaussian" && family$link != "identity") {
      stop("For 'LM' and 'LMM', are only available for continuous outcomes")
    }
  } 
  ...
  model <- switch(method,
                  ...
                  "GLMM" =  try(glmer(paste(formulai, "+ (1 | clus_id)"), data = data1, family = family, nAGQ = 0),silent = T),
