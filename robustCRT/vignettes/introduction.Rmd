...
The \`robustCRT\`\` package provides tools for computing the model-robust 
...
The primary data fitting function is `robust_CRT`, which generates a
...
User can call the `robust_CRT` function as follows:

`robust_CRT <- function(formula, data, clus_id, trt, prob, method,family,corstr,scale, jack, alpha)`
...
In this example, we demonstrate how to use the `robust_CRT` function to
